...
`);var y=function(d){(function(p,h){if(typeof h!="function"&&h!==null)throw new TypeError("Super expression must either be null or a function");p.prototype=Object.create(h&&h.prototype,{constructor:{value:p,writable:!0,configurable:!0}}),h&&f(p,h)})(g,d);var l,r,v,m,k=(l=g,r=u(),function(){var p,h=o(l);if(r){var E=o(this).constructor;p=Reflect.construct(h,arguments,E)}else p=h.apply(this,arguments);return n(this,p)});function g(){return function(p,h){if(!(p instanceof h))throw new TypeError("Cannot call a class as a function")}(this,g),k.call(this)}return v=g,(m=[{key:"connectedCallback",value:function(){var p=this.attachShadow({mode:"open"});p.appendChild(b.content.cloneNode(!0)),this.getAttribute("year")===null?p.getElementById("year").innerHTML=new Date().getFullYear():p.getElementById("year").innerHTML=this.getAttribute("year")}}])&&i(v.prototype,m),g}(a(HTMLElement));window.customElements.define("copyright-x",y)}]);async function S(){return await(await fetch("https://api.github.com/users/hybridx")).json()}async function L(){return await(await fetch("https://api.github.com/users/hybridx/repos?sort=updated&per_page=6&fork=false")).json()}function M(){const e=document.documentElement,t=(e.getAttribute("data-theme")||"dark")==="dark"?"light":"dark";e.setAttribute("data-theme",t),localStorage.setItem("theme",t),x(t)}function x(e){const s=document.getElementById("theme-toggle");s&&(s.innerHTML="",e==="dark"?s.innerHTML=`
    ...
    `)}function j(){const t=localStorage.getItem("theme")||"dark";document.documentElement.setAttribute("data-theme",t),x(t)}function C(){const e=document.querySelectorAll(".fade-in"),s=new IntersectionObserver(t=>{t.forEach(i=>{i.isIntersecting&&i.target.classList.add("visible")})},{threshold:.1});e.forEach(t=>{s.observe(t)})}function P(){const e=["Software Engineer","DevOps Enthusiast","Homelab Explorer","Open Source Contributor"],s=document.querySelector(".typing-text");if(!s)return;let t=0,i=0,n=!1,a=100;function c(){const u=e[t];n?(s&&(s.textContent=u.substring(0,i-1)),i--,a=50):(s&&(s.textContent=u.substring(0,i+1)),i++,a=100),!n&&i===u.length?setTimeout(()=>n=!0,2e3):n&&i===0&&(n=!1,t=(t+1)%e.length),setTimeout(c,a)}c()}function T(){const e=document.createElement("canvas");e.id="particle-canvas",e.style.position="fixed",e.style.top="0",e.style.left="0",e.style.width="100%",e.style.height="100%",e.style.pointerEvents="none",e.style.zIndex="1",document.body.insertBefore(e,document.body.firstChild);const s=e.getContext("2d");let t=[];function i(){e.width=window.innerWidth,e.height=window.innerHeight}function n(){return{x:Math.random()*e.width,y:Math.random()*e.height,vx:(Math.random()-.5)*.5,vy:(Math.random()-.5)*.5,size:Math.random()*2+1,opacity:Math.random()*.5+.1}}function a(){t=[];for(let o=0;o<50;o++)t.push(n())}function c(){t.forEach(o=>{o.x+=o.vx,o.y+=o.vy,(o.x<0||o.x>e.width)&&(o.vx*=-1),(o.y<0||o.y>e.height)&&(o.vy*=-1)})}function u(){s&&(s.clearRect(0,0,e.width,e.height),t.forEach(o=>{s&&(s.beginPath(),s.arc(o.x,o.y,o.size,0,Math.PI*2),s.fillStyle=`rgba(100, 108, 255, ${o.opacity})`,s.fill())}),t.forEach((o,b)=>{t.slice(b+1).forEach(y=>{if(!s)return;const d=Math.sqrt(Math.pow(o.x-y.x,2)+Math.pow(o.y-y.y,2));d<100&&(s.beginPath(),s.moveTo(o.x,o.y),s.lineTo(y.x,y.y),s.strokeStyle=`rgba(100, 108, 255, ${.1*(1-d/100)})`,s.lineWidth=1,s.stroke())})}))}function f(){c(),u(),requestAnimationFrame(f)}i(),a(),f(),window.addEventListener("resize",()=>{i(),a()})}function O(){const e=document.querySelectorAll(".skill-item"),s=new IntersectionObserver(t=>{t.forEach(i=>{if(i.isIntersecting){const n=i.target.querySelector(".skill-progress");if(n){const a=n.getAttribute("data-progress");a&&setTimeout(()=>{n.style.width=a+"%"},200)}}})},{threshold:.5});e.forEach(t=>{s.observe(t)})}function I(){const e=document.querySelectorAll(".timeline-item"),s=new IntersectionObserver(t=>{t.forEach(i=>{i.isIntersecting&&i.target.classList.add("visible")})},{threshold:.3});e.forEach(t=>{s.observe(t)})}function z(){const e=document.getElementById("loading-screen"),s=document.querySelector(".portfolio"),t=["Loading amazing content...","Fetching GitHub data...","Initializing animations...","Almost ready...","Welcome!"];let i=0;const n=document.querySelector(".loading-message"),a=setInterval(()=>{i<t.length-1&&(i++,n&&(n.textContent=t[i]))},800);setTimeout(()=>{clearInterval(a),e&&e.classList.add("hidden"),s&&s.classList.add("loaded"),setTimeout(()=>{e&&e.remove()},800)},3e3)}function B(){document.querySelectorAll('a[href^="#"]').forEach(s=>{s.addEventListener("click",t=>{var a;t.preventDefault();const i=s.getAttribute("href");if(!i)return;const n=document.querySelector(i);if(n){const c=((a=document.querySelector("header"))==null?void 0:a.offsetHeight)||80,u=n.getBoundingClientRect().top+window.scrollY-c;window.scrollTo({top:u,behavior:"smooth"})}})});const e=document.querySelector("header");if(e){let s=!1;const t=()=>{const i=window.scrollY,n=Math.min(.95,.7+i/300*.25);e.style.background=`rgba(${getComputedStyle(document.documentElement).getPropertyValue("--header-bg").replace(/[^\d,]/g,"")}, ${n})`,s=!1};window.addEventListener("scroll",()=>{s||(requestAnimationFrame(t),s=!0)},{passive:!0})}}function _(){const e=document.querySelectorAll("img[data-src]"),s=new IntersectionObserver(n=>{n.forEach(a=>{if(a.isIntersecting){const c=a.target;c.src=c.dataset.src,c.classList.remove("lazy"),s.unobserve(c)}})});e.forEach(n=>{s.observe(n)});const t=document.createElement("link");t.rel="preload",t.as="font",t.crossOrigin="anonymous",document.head.appendChild(t);const i=document.createElement("link");i.rel="preconnect",i.href="https://api.github.com",document.head.appendChild(i)}function A(){if(window.matchMedia("(prefers-reduced-motion: reduce)").matches){document.documentElement.style.setProperty("--animation-duration","0s"),document.documentElement.style.setProperty("--transition-duration","0s");const t=document.getElementById("particle-canvas");t&&(t.style.display="none")}document.addEventListener("keydown",t=>{t.key==="Tab"&&document.body.classList.add("keyboard-navigation")}),document.addEventListener("mousedown",()=>{document.body.classList.remove("keyboard-navigation")});const s=document.createElement("a");s.href="#hero",s.className="skip-link",s.textContent="Skip to main content",s.setAttribute("aria-label","Skip to main content"),document.body.insertBefore(s,document.body.firstChild)}function $(){window.addEventListener("error",e=>{console.error("Portfolio Error:",e.error);const s=document.getElementById("loading-screen");s&&s.classList.add("hidden");const t=document.querySelector(".portfolio");t&&t.classList.add("loaded")}),window.addEventListener("unhandledrejection",e=>{console.error("Unhandled Promise Rejection:",e.reason)})}function H(){window.addEventListener("load",()=>{setTimeout(()=>{const e=performance.getEntriesByType("navigation")[0];if(e){const s={dns:e.domainLookupEnd-e.domainLookupStart,tcp:e.connectEnd-e.connectStart,request:e.responseStart-e.requestStart,response:e.responseEnd-e.responseStart,domLoading:e.domContentLoadedEventEnd-e.fetchStart,domComplete:e.loadEventEnd-e.fetchStart};console.log("Performance Metrics:",s)}},0)})}async function D(){var n;const e=await S(),s=await L();j(),document.querySelector("#app").innerHTML=`
